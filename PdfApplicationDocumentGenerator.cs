    ...
    public class PdfApplicationDocumentGenerator : IApplicationDocumentGenerator
    {
            ...
            ILogger<PdfApplicationDocumentGenerator> logger)
        {
            try
            {
                if (dataContext != null)
                {
                    ...
                    if (logger == null)
                    {
                        logger.LogWarning("logger is null");
                        return;
                    }
                    else
                    {
                    ...
                    if (logger == null && _templatePathProvider == null)
                    {
                        logger.LogWarning("templatePathProvider is null");
                        return;
                    }
                    else
                    {
                ...
                {
                    _logger.LogWarning("DataContext should not be null");                 
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning($"An error occured '{ex.Message}'");
            }
        }
        ...
        public byte[] Generate(Guid applicationId, string baseUri)
        {
            ...
            if (application != null)
            {
                ...
                try
                {
                   pdf = _pdfGenerator.GenerateFromHtml(view, pdfOptions);                  
                }
                catch (Exception ex)
                {
                    _logger.LogWarning($"An error occured while generating the pdf '{ex.Message}'");
                    return null;
                }
                return pdf.ToBytes();
            }
